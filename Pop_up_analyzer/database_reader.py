...
async def main(on_file_downloaded=None):
    ...
    try:
        ...
        for file_data in existing_files:
            ...
            dest_path = os.path.join(LOCAL_DIR, "audio.wav")  # Always use the same filename
    ...
    async def handle_update(payload):
        ...
        print(f"Realtime {event_type} event received for {event_data.get('table')}")

        data = event_data.get("record", {})  # New record data
        old_data = event_data.get("old_record", {})  # Old record data (for updates)

        # Only process INSERT events for new files, or UPDATE if object_path changed
        if event_type == "INSERT":
            print("Processing new file INSERT")
        elif event_type == "UPDATE":
            old_path = old_data.get(OBJECT_PATH_COLUMN)
            new_path = data.get(OBJECT_PATH_COLUMN)
            if old_path == new_path:
                print("UPDATE received but object_path unchanged; skipping re-download")
                return
            else:
                print("Processing file path UPDATE")
        else:
            print(f"Ignoring {event_type} event")
            return
        
        new_updated_at = data.get(UPDATED_AT_COLUMN)
        if new_updated_at is None:
            print("Event received but updated_at missing:", payload)
            return

        object_name = data.get(OBJECT_PATH_COLUMN)
        if not object_name:
            print("Event received but object name missing:", payload)
            return
    ...
    # Subscribe to all changes on wav_files
    ...
    try:
        ...
        print("Successfully subscribed to wav_files changes.")
    except Exception as e:
